...
class FeatureEngineer:
    ...
    def extract_features(self, payload: Dict[str, Any]) -> pd.Series:
        ...
        features['receiver_agencia'] = self._encode_agencia(receiver.get('agencia', '0001'))
        ...
        if timestamp_str:
            try:
                ...
                features['horario_noturno'] = 1 if timestamp.hour >= 22 or timestamp.hour < 6 else 0
                
                # Cyclical encoding for temporal features
                features['hora_sin'] = math.sin(2 * math.pi * timestamp.hour / 24)
                features['hora_cos'] = math.cos(2 * math.pi * timestamp.hour / 24)
                features['dia_semana_sin'] = math.sin(2 * math.pi * timestamp.weekday() / 7)
                features['dia_semana_cos'] = math.cos(2 * math.pi * timestamp.weekday() / 7)
        ...
        features['valor_zscore'] = valor / 5000 if valor > 0 else 0  # Normalizado por valor médio
        ...
        if cpf_sender:
            features = self._extract_behavioral_features(features, cpf_sender, timestamp_str)
        else:
        ...
        features['cross_border'] = 1 if sender_banco and receiver_banco and sender_banco != receiver_banco else 0
        features['mesmo_banco'] = 1 if sender_banco and receiver_banco and sender_banco == receiver_banco else 0
        features['banco_diferente_sender'] = 1 if receiver_banco and receiver_banco > 0 and receiver_banco != sender_banco else 0
        ...
        features['transacao_fora_horario_comercial'] = 1 if features['hora_do_dia'] < 8 or features['hora_do_dia'] >= 18 else 0
        features['valor_atipico'] = features['valor_maior_5000']
        features['multiplos_dispositivos'] = 0  # Placeholder - would require device tracking
    ...
    def _extract_behavioral_features(self, features: dict, cpf_sender: str, timestamp_str: str) -> dict:
        ...
        current_time = datetime.fromisoformat(timestamp_str.replace('Z', '+00:00')) if timestamp_str else datetime.now()
        ...
        if len(self.transaction_history[cpf_sender]) > 100:
            self.transaction_history[cpf_sender] = self.transaction_history[cpf_sender][-100:]
        ...
        if receiver_banco and receiver_banco > 0:
            ...
            features['nova_relacao'] = 1 if receiver_banco not in previous_receivers else 0
        else:
            features['nova_relacao'] = 0
