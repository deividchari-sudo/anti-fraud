...
class FraudDetectionModel:
    def __init__(self, model_path: Optional[str] = None, model_repository: Optional[ModelRepository] = None):
        ...
        self.model_repository = model_repository or JoblibModelRepository(
            self.model_path,
            Path(self.model_path).parent / 'feature_names.json'
        )
    ...
    def train(self, df: pd.DataFrame, target_col: str = 'fraudResult') -> dict:
        ...
        print(f"After SMOTE - Training set: {X_train_resampled.shape}, Fraud distribution: {y_train_resampled.mean():.4f}")
    ...
    def predict(self, features: pd.DataFrame, transaction_id: str = None) -> Tuple[float, bool, Optional[Dict[str, Any]]]:
    ...
    def _get_explanation(self, X: pd.DataFrame, fraud_probability: float) -> Dict[str, Any]:
        ...
        try:
            ...
            if isinstance(shap_values, list):
                shap_values = shap_values[0]  # Take first element for binary classification
